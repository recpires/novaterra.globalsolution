...
export default function Assistente() {
  ...
  const send = async (text) => {
    if (!text.trim()) return;
    const userMsg = { id: msgs.length + 1, role: 'user', text, ts: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) };
    ...
    <div className="flex flex-col h-full" style={{ maxHeight: 'calc(100vh - 64px - 40px)' }}>
        ...
        {msgs.map((m) => (
          <motion.div key={m.id}
            ...
            <div className={`max-w-[70%] space-y-1 ${m.role === 'user' ? 'items-end' : 'items-start'} flex flex-col`}>
              ...
              {m.action && (
                <button className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-xl transition-all hover:brightness-110"
      ...
      <div className="flex gap-2 flex-wrap pb-3 pt-1">
        {CHIPS.map(c => (
          <button key={c.label} onClick={() => send(c.label)}
            ...
            style={{ background: C.card, color: C.txt2, border: `1px solid ${C.border}` }}>
      ...
      <div className="flex gap-3 pt-3" style={{ borderTop: `1px solid ${C.border}` }}>
        ...
        <button onClick={() => send(input)} disabled={!input.trim() || typing}
          className="w-11 h-11 rounded-xl flex items-center justify-center transition-all hover:brightness-110 disabled:opacity-40"
