...
import {
  ...
  Brain, FileText, Settings, LogOut, Bell, User, Menu, X, ChevronRight,
} from 'lucide-react';
import { C } from '../theme';

const NAV = [
  { path: '/dashboard',  label: 'Dashboard',      Icon: LayoutDashboard, color: C.blue   },
  { path: '/agrisat',    label: 'AgriSat',         Icon: Leaf,            color: C.teal   },
  { path: '/sentinel',   label: 'SentinelAlert',   Icon: ShieldAlert,     color: C.orange },
  { path: '/ecotrack',   label: 'EcoTrack',        Icon: TreePine,        color: C.green  },
  { path: '/assistente', label: 'Assistente IA',   Icon: Brain,           color: C.purple },
  { path: '/relatorios', label: 'Relatórios',      Icon: FileText,        color: C.txt2   },
];
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{ width: 240, background: C.card, borderRight: `1px solid ${C.border}` }}
      >
        {/* Logo */}
        <div className="h-16 flex items-center gap-2.5 px-5" style={{ borderBottom: `1px solid ${C.border}` }}>
        ...
        {/* Bottom */}
        <div className="px-3 pb-5 space-y-1" style={{ borderTop: `1px solid ${C.border}`, paddingTop: 12 }}>
          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all hover:bg-white/5"
            style={{ color: C.txt2 }}>
            <Settings size={17} /> Configurações
          </button>
          <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all hover:bg-red-500/10"
            style={{ color: C.red }}>
            <LogOut size={17} /> Sair
          </button>
        </div>
...
function Header({ onMenuClick }) {
  ...
  return (
    <header className="h-16 flex items-center px-5 gap-4"
      ...
      {/* User */}
      <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl cursor-pointer hover:bg-white/5 transition-all">
        <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: `${C.teal}22`, border: `1px solid ${C.teal}44` }}>
